...
RSpec.feature "Govspeak preview", type: :feature do
  scenario "Add Govspeak for one title" do
    ...
    expect(page).to have_css(".gem-c-govspeak h2", text: "Ut viverra ante nunc, quis efficitur leo volutpat et")
  end
  ...
  scenario "Add Govspeak for one title and one paragraph" do
    ...
    expect(page).to have_css(".gem-c-govspeak h2", text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit")
    expect(page).to have_css(".gem-c-govspeak p", text: "Ut mattis augue sed nisl tincidunt, a laoreet lectus consequat. Mauris lacinia venenatis urna eu rhoncus.")
  end
  ...
  scenario "Add Govspeak for one paragraph and three list items" do
    ...
    expect(page).to have_css(".gem-c-govspeak p", text: "Pellentesque ut facilisis eros, eget viverra nulla.")
    expect(page).to have_css(".gem-c-govspeak li", count: 3)
    expect(page).to have_css(".gem-c-govspeak li:nth-child(1)", text: "Sed porta et nisi et tempus")
    expect(page).to have_css(".gem-c-govspeak li:nth-child(2)", text: "Nullam nec quam fringilla, imperdiet sem nec, feugiat urna. In hac habitasse platea dictumst")
    expect(page).to have_css(".gem-c-govspeak li:nth-child(3)", text: "Mauris egestas, turpis a lobortis fringilla, orci neque pellentesque tellus, a rutrum sem lorem ac nibh.")
  end
  ...
  scenario "Add Govspeak for one paragraph and one link" do
    ...
    expect(page).to have_css(".gem-c-govspeak p", text: "Nulla finibus finibus hendrerit. In hac habitasse platea dictumst. Nam volutpat quam sed tellus ullamcorper ultrices. Maecenas hendrerit dolor lacus, sed commodo lectus sodales sed.")
